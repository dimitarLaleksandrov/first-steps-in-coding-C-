...
describe('E2E tests', function () {
	...
	beforeEach(async () => {
		...
		await context.route(
			'**' + endpoints.details + '*',
			route => route.fulfill(json(mockData[0]))
		)
	...
	describe('Authentication [ 20 Points ]', () => {
		...
		it('register makes correct API call [ 5 Points ]', async () => {
			...
			page.route(
				endpoint,
				route => route.fulfill(json({ _id: '0001', email, accessToken: 'AAAA' }))
			)
	...
	describe('Catalog [ 25 Points ]', () => {
		...
		it('show meme details [ 5 Points ]', async () => {
			...
			await page.route(
				'**' + endpoints.details + '*',
				route => route.fulfill(json(mockData[3]))
			)
	...
	describe('CRUD [ 40 Points ]', () => {
		...
		it('delete makes correct API call for logged in user [ 5 Points ]', async () => {
			...
			const [request] = await Promise.all([
				page.waitForRequest('**' + endpoints.delete +
					'74463e5b-b893-44e8-bd14-5fc8feeddb94'),
		...
		it('edit does NOT work with empty fields [ 5 Points ]', async () => {
			...
			await page.route(
				'**' + endpoints.details + '*',
				route => route.fulfill(json(mockData[5]))
			)
	...
	describe('User Profile Page [ 10 Points ]', async () => {
		...
		it(
			'check profile page for "No memes in database." - with 0 memes [ 2.5 Points ]',
			async () => {
				await page.waitForTimeout(300)
		...
		it('check profile page information - with 2 memes [ 2.5 Points ]', async () => {
			await page.route(
				'**' + endpoints.profile,
				route => route.fulfill(json([mockData[0], mockData[1]]))
			)
			...
			const memes = await page.$$eval(
				'.user-meme-listings .user-meme',
				p => p.map(p => p.textContent)
			)
			await page.waitForTimeout(300)
